...
(function() {
  ...
  function createDocument(options) {
    var timer = Debug.startPerformanceTimer ? Debug.startPerformanceTimer('createPDFDocument') : null;
    ...
    try {
      ...
      doc.setFontSize(pdfConfig.defaultFontSize);
  ...
  function addWatermark(doc) {
    ...
    try {
      ...
      for (var i = 1; i <= totalPages; i++) {
        ...
        var watermarkTextWidth = doc.getTextWidth(pdfConfig.watermark);
        doc.text(pdfConfig.watermark, pageWidth - watermarkTextWidth - 10, pageHeight - 10);
      }
    } catch (error) {
  ...
  function addParagraphTitle(doc, title, y) {
    ...
    try {
      ...
      doc.text(title, pdfConfig.margins.left, y);
  ...
  function addSignatureFields(doc, city) {
    ...
    try {
      ...
      var leftColumnX = pdfConfig.margins.left;
  ...
  // PDF-Generator-Modul definieren
  var PDFGenerator = {
    init: init,
    ...
    setWatermark: function(text) {
      pdfConfig.watermark = text;
    },
    setMargins: function(margins) {
      pdfConfig.margins = Object.assign(pdfConfig.margins, margins);
    }
  };
  
  // PDF-Generator-Modul global verfügbar machen
  window.ContractGenerator = window.ContractGenerator || {};
  window.ContractGenerator.PDFGenerator = PDFGenerator;
})();
